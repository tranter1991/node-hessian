...
var _lengthOfUTF8 = function (head) {
	if (head < 0x80) {
		return 1;
	}
	if ((head & 0xe0) === 0xc0) {
...
Reader.prototype._readUTF8String = function (len) {
	var startPos = this.reader.tell();
	var head;
	var l;
	//console.log('read utf8 string tunk, chars length: %d', len);
	...
	while (len--) {
		head = this.reader.nextUInt8();
		l = _lengthOfUTF8(head);
		this.reader.move(l - 1);
	}

	l = this.reader.tell() - startPos;

	//console.log('utf8 length: %d', l);

	this.reader.seek(startPos);

	return this.reader.nextString(l);
}
...
Reader.prototype.readString = function(data) {
    ...
    if (code >= 0 && code < 32) {
        ...
        var len = this.reader.nextUInt16BE();
	    return this._readUTF8String(len) + this.readString();
